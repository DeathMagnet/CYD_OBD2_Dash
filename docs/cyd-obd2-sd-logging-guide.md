...
- **Page 5 log management.** `CsvLogger::getLogSummary()` scans the SD root for a file count and total byte size, rendered on Page 5 alongside a "DELETE ALL LOGS" button (`CsvLogger::deleteAllLogs()`, tap-to-confirm within 5 seconds) that closes the active file, removes every session log, and immediately opens a fresh one — but only resumes writing if logging was actually active beforehand, so tapping it with `SD_LOGGING_ENABLED` unset can't silently start a session.
