...
- When regenerating a boot/splash image asset (e.g. `boot_logo_png.h`), verify the decoded pixels against the source image (e.g. compare average per-channel values, or diff every pixel against the source array when re-encoding from an existing one) rather than trusting the conversion tool — a truncated, mis-packed, or lossy-quantized asset will compile and run but render with an incorrect tint or banding. `pushImage()` also requires `setSwapBytes(true)` on this panel for a standard (non-pre-swapped) RGB565 buffer, whether pushed directly or produced by `PNGdec`'s `getLineAsRGB565()`; `fillScreen`/`fillRect` do not need it because they swap bytes internally.
