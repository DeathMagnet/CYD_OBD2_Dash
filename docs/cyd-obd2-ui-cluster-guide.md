...
The dashboard displays all five dashboard pages (1–5: Primary Cluster, Engine Load, Car-Specific, Performance, Diagnostics) plus four dedicated config pages (UI, GAUGES, USER VARS, LOGS), with these deliberate deviations from the design above:
...
- **All four themes are implemented.** `getTheme()` (`src/display/theme.cpp`) returns the S197 - Digital, S197 - Analog, Neon, or Modern Flat palette for the corresponding `ThemeId`. The UI config page's "Active Theme" row has a tap-to-cycle button (mirroring the Units row) that rotates through all four themes (Modern Flat -> Neon -> S197 - Digital -> S197 - Analog -> Modern Flat), persists the choice, and immediately recolors the visible page. **S197 - Analog is the default theme** on a device with no saved `/config.txt`; any previously saved theme choice is always honored on reboot (via `config::kDefaultThemeId` and `ConfigStore::parseLine()`'s theme range validation).
...
The UI supports four distinct visual themes switchable at runtime or persisted via config.

### 1. S197 - Digital Theme
...
- **Gauge Bezel**: A decorative 4px-thick chrome ring drawn just outside each round gauge (RPM, Speed, Engine Load, Vacuum/Boost) — not shown on Neon or Modern Flat.
- **Typography**: Retro-block / racing numerals (TFT_eSPI Font 7, 7-segment LED look), tick marks every 500 RPM / 10 MPH, animated color-fill arc indicating the live value.

### 2. S197 - Analog Theme
- **Inspiration**: Same 2005–2010 Ford Mustang S197 cluster as S197 - Digital, restyled as a traditional needle gauge instead of an LED-backlit digital one.
- **Color Palette**: Same navy background, chrome bezel, LED-green center numeral (`textPrimary`, same as S197 - Digital, shift-light flash still overriding to `warningActive` red), and vibrant red needle as S197 - Digital; the tick number labels (below) are always plain white regardless of theme colors.
- **Needle instead of fill arc**: RPM, Speed, and Engine Load render a needle (`gaugewidgets::eraseNeedle()`/`drawNeedle()`) sweeping over a static dial face, in place of the other themes' animated color-fill arc. The caution/danger redline zone still paints directly on the dial face (as on a real OEM gauge), and the needle itself indicates the live value. Drawn as a two-step erase/draw pair each frame with the numeral and tick labels redrawn in between, so the needle always ends up painted on top of both, never hidden behind them. `eraseNeedle()` skips the erase entirely when the needle's rounded angle hasn't changed since the last draw (a steady reading would otherwise erase-then-redraw an unmoved needle every ~100ms tick, which reads as flicker), and erases with a slightly wider line than `drawNeedle()` draws, so integer-rounding differences between the two can't leave a stale sliver behind.
- **Labeled major ticks**: Major tick marks are numbered in white (`gaugewidgets::drawGaugeTickLabels()`) — e.g. "1".."9" around the RPM dial (in thousands, `FreeSansBold9pt7b` — bold and legible at the small size a dial's tick labels need, already bundled with TFT_eSPI so no new font asset is required), "10".."140" every 10 around the speedometer (S197 - Analog's own finer tick graduation, `config::kSpeedTickIntervalMinorAnalog`/`kSpeedTickIntervalMajorAnalog` — using the small built-in GLCD font instead, since that many labels are too dense for the bold free font), "20".."100" every 20% around Engine Load (which the other three themes don't draw tick marks for at all). Labeling starts at the first major interval past zero (no "0" label), matching how the far end is already excluded by `excludeFromValue`. Ticks stay permanently silver/white as described above (never lighting up as the value passes them), giving them the look of fixed, printed markings on the dial face. Redrawn every Dynamic tick rather than once statically, since the needle's tip reaches close enough to their radius that a one-time draw would risk permanent erase damage.
- **Smaller, relocated center numeral**: The digital numeral (and its unit caption) shrinks from S197 - Digital's Font 7 to a smaller free font (`FreeSansBold9pt7b`) and moves down into the lower half of the dial, so it no longer sits on top of the needle's pivot.
...
### 5. All Themes
- **Redline Arc/Zone**: Starting at the GAUGES config page's Shift Light RPM (default 5800) up to Redline RPM (default 6200), then held at full redline color to the end of the sweep — a color-fill arc segment for S197 - Digital/Neon/Modern Flat, or a static zone painted on the dial face behind the needle for S197 - Analog.

### Implementation Structure (`src/display/theme.h`)

`ThemeId` (`S197Digital = 0`, `Neon = 1`, `ModernFlat = 2`, `S197Analog = 3`) selects a `const ThemeColors&` from `getTheme()`. `ThemeColors` holds every themeable color (background, panel, gauge arc/needle/bezel/text colors, tick colors, warning/highlight/save-button/connection-badge colors) plus a handful of per-theme booleans that toggle whole visual features on or off (`showGaugeBezel`, `showOuterTicks`, `useNeedleGauge`) and the display name shown on the Config: UI page. Treat `src/display/theme.h` as the authoritative field list rather than a copy here — it has grown several times as themes gained features (most recently the S197 - Analog needle-gauge work), and a duplicated struct in this doc is exactly what goes stale.
...
The OBD-II adapter's Bluetooth identity (`mac`/`id`/`password`) is not part of this system — there is no config page for it. It's required to be present and complete in a separate `/obd_config.txt` file on the SD card, read once at boot; a missing or incomplete file halts boot with an on-screen error instead of falling back to a default. See [CYD OBD-II Dashboard Implementation Guide](cyd-obd2-dashboard-implementation.md#obd-ii-integration) and the template at `docs/sd_card_templates/obd_config.example.txt`.
...
| --- | --- | --- | --- |
| **Active Theme** | Modern Flat, Neon, S197 - Digital, S197 - Analog | S197 - Analog | UI visual style; tap-to-cycle button rotates through all four themes |
...
- [ ] Theme switching (Modern Flat -> Neon -> S197 - Digital -> S197 - Analog -> Modern Flat, via the UI page's Active Theme cycle button) immediately recolors gauges, bezels, needles, and text, and the selection survives Save + reboot.
