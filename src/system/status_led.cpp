...
void StatusLed::update(bool checkEngineOn, bool shiftLightOn, uint16_t warningColorRgb565, uint32_t nowMs) {
    ...
    if (shiftLightOn) {
        // The shift-point flash owns the LED while active, alternating
        // cleanly between the warning color and off, so it stays visibly
        // distinct from (and isn't masked by) a steady MIL red below.
        if (flashPhase) {
