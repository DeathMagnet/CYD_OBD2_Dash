...
- **Page 6 — Diagnostics**: Read, decode, and clear OBD-II Diagnostic Trouble Codes (DTCs). Tapping the Check Engine Light (MIL) on any page jumps here.
...
- **UI**: Tap-to-cycle "Active Theme" button rotates through Modern Flat -> Neon -> S197, persisting the choice and recoloring the visible page immediately.
...
**S197 theme**: Its warning color (`0xF8C0`, amber-orange) quantizes down to the LED's 3 fixed on/off channels as pure red (red channel over threshold, green and blue under), so — like Modern Flat — the shift flash and steady MIL both render the same red; only the flash cadence tells them apart.
